...
export async function POST(request) {
  ...
  try {
    const rows = await sql`SELECT id, price FROM cells WHERE id = ANY(${ids}) AND (status = 'available' OR (status = 'reserved' AND reserved_until < now())) FOR UPDATE`;
    if (rows.length !== ids.length) return Response.json({ error: 'Některá vybraná pole už nejsou dostupná. Obnov stránku a vyber je znovu.' }, { status: 409 });

    const total = rows.reduce((sum, row) => sum + Number(row.price), 0);
    await sql`INSERT INTO reservations (id,name,email,message,image_data,total_amount,expires_at) VALUES (${reservationId},${name},${email},${message || null},${imageData},${total},${expiresAt.toISOString()})`;
    await sql`UPDATE cells SET status='reserved', reservation_id=${reservationId}, reserved_until=${expiresAt.toISOString()} WHERE id = ANY(${ids})`;
    await sql`INSERT INTO reservation_cells (reservation_id,cell_id) SELECT ${reservationId}, id FROM cells WHERE id = ANY(${ids})`;
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { reservationId, cellCount: String(ids.length) },
